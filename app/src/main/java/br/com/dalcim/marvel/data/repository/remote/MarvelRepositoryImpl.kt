...
class MarvelRepositoryImpl(private val retrofit: Retrofit) : MarvelRepository {
    ...
    override fun getComics(characterId: Long, onSuccess: (List<Comic>) -> Unit, onFailure: (String) -> Unit) {
        api.getCharacterComics(characterId).exec(onSuccess, onFailure) {
            it.orEmpty().map(::Comic)
        }
    }
}
